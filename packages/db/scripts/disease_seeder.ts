import { dbTransact as db } from "../index";
import {
  conditions,
  categories,
  bodyParts,
...
async function seed() {
  console.log("🚀 Starting Disease Seeder...");
  ...
  "Trigger thumb or finger in children and young people": {
    "isSystemic": false,
    "bodyParts": [
      "Hand and Fingers"
    ]
  ...
  "Yellow fever": {
    ...
    await db.delete(conditionToBodyParts);
    await db.delete(conditionToCategories);
    await db.delete(conditions);

    // 3. Fetch categories and body parts for lookup
    console.log("📦 Fetching categories and body parts for lookup...");
    const allCategories = await db.select().from(categories);
    const allBodyParts = await db.select().from(bodyParts);
    ...
    for (const disease of seedData.diseases_seed) {
      const research = diseaseMapping[disease.name];
      if (!research) {
        console.warn(`⚠️ No research data found for: ${disease.name}. Skipping.`);
        continue;
      }

      try {
        // Insert Condition
        const [insertedCondition] = await db.insert(conditions).values({
          name: disease.name,
          slug: disease.slug,
          nhsLink: disease.nhs_link,
          ...
          contactYourDoctor: disease.contact_your_doctor,
          moreInformation: disease.more_information,
          attribution: disease.attribution,
          isSystemic: research.isSystemic,
        ...
        const categoryId = categoryMap.get(disease.category_association_slug);
        if (categoryId) {
          await db.insert(conditionToCategories).values({
            conditionId: insertedCondition.id,
            categoryId: categoryId,
          }).onConflictDoNothing();
        } else {
          console.warn(`⚠️ Category not found: ${disease.category_association_slug} for ${disease.name}`);
        }

        // 6. Associate Body Parts (including parents)
        const associatedBodyPartIds = new Set<string>();

        for (const bpName of research.bodyParts) {
          const bp = bodyPartNameMap.get(bpName.toLowerCase());
          if (bp) {
            // Add the part itself
            associatedBodyPartIds.add(bp.id);

            // Trace parents using path
            const pathParts = bp.path.split(".");
            for (let i = 1; i <= pathParts.length; i++) {
              const parentPath = pathParts.slice(0, i).join(".");
              const parentId = bodyPartPathMap.get(parentPath);
              if (parentId) {
                associatedBodyPartIds.add(parentId);
              }
            }
          } else {
            console.warn(`⚠️ Body part not found in DB: ${bpName} for ${disease.name}`);
          }
        }

        for (const bpId of associatedBodyPartIds) {
          await db.insert(conditionToBodyParts).values({
            conditionId: insertedCondition.id,
            bodyPartId: bpId,
          }).onConflictDoNothing();
        }
        ...
        console.error(`❌ Error seeding ${disease.name}:`, err);
      }
    }
