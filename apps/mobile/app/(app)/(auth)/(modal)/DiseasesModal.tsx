...
const DiseasesModal = () => {
  ...
  const handleLetterPress = (letter: string) => {
    ...
    setSelectedParent(null); // Clear category view
  ...
  const displayCategories = useMemo(() => {
    if (!selectedParent) return categories?.filter((c) => !c.parent_id) || [];
    ...
    <View className="flex-1 bg-white">
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="flex-1 px-6">
        ...
        {searchQuery.length > 0 ? (
          <View className="py-4">
            <Text className="font-bold text-slate-400">SEARCH RESULTS</Text>
          </View>
        ) : (
          <>
            ...
            <View className="mt-2">
              <View className="flex-row items-center justify-between mb-4">
                ...
                {/* Back button should clear everything */}
                {(selectedParent || isFiltered) && (
                  <TouchableOpacity
                    onPress={() => {
...
const AlphabetGrid = ({
  ...
  return (
    // Fragment ensures valid React return without adding a View that breaks flex-wrap
    <>
      {alphabet.map((letter) => (
        <TouchableOpacity
          key={letter}
          ...
          className="bg-slate-50 border border-slate-100 rounded-3xl items-center justify-center shadow-sm active:bg-emerald-50 active:scale-95 transition-all"
        >
