...
export default function FacilityDetails() {
  ...
  const { data: media, isLoading: isMediaLoading } = useGetMediaPublicUrl(
    facility?.media_urls || [],
  );
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      >
        ...
        <TouchableOpacity className="w-10 h-10 items-center justify-center rounded-2xl bg-white/90 shadow-sm">
        ...
        >
          {(() => {
            ...
            if (media && media.length > 0) {
              return media.map((url: string, index: number) => (
                <Image
                  key={url}
                  source={{ uri: url }} // No need for getPublicUrl helper here, the hook did it
                  style={{ width, height: 300 }}
                  contentFit="cover"
                />
              ));
